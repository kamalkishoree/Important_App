...
function reInitMap() {
    ...
    //new code for route
    var i = 0;
    $("input[name='agent_ids[]']").each(function (index, element) {
        i++;
        ...
        if($("#agent_task_details_"+agentid).val()!='')
        {
            var al_task = JSON.parse($("#agent_task_details_"+agentid).val());
        }else{
            var al_task = [];
        }
        if($("#agent_driver_details_"+agentid).val()!='')
        {
            var agent_locatn = JSON.parse($("#agent_driver_details_"+agentid).val());
...
function calculateAndDisplayRoute(directionsService, directionsRenderer, map, alltask, agent_location) {
    ...
    var origin_lat = parseFloat(agent_location.lat);if(isNaN(origin_lat)){origin_lat = parseFloat(alltask[0].latitude);}
    var origin_long = parseFloat(agent_location.long);if(isNaN(origin_long)){origin_long = parseFloat(alltask[0].longitude);}
    
    directionsService.route({
            origin: new google.maps.LatLng(parseFloat(origin_lat), parseFloat(agent_location.long)),
