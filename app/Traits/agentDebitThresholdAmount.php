                ...
                if ($orders) {
                    foreach ($orders as $order) {
                        ...
                        if ($cod >= $threshold_amount) {
                            ...
                            $threshold_type                     = $type;
                            $status                             = 1;
                            $data                               = ['agent_id' => $agent->id, 'threshold_amount' => $threshold_amount, 'transaction_id' => $transaction_id, 'date' => $date, 'payment_type' => $payment_type, 'threshold_type' => $threshold_type, 'status' => $status];
                            $this->SaveCollectAmount($data);
                        } else {
                            Agent::where('id', $order->driver_id)->update(['is_available' => 0]);
                        }
                    }
                }
            }
