    ...
    public function updateTaskStatus(Request $request)
    {
        ...
        switch ($request->task_status) {
            ...
            case 4:
                ...
                $errorMsgOtp = '';
                if (!empty($orderId->tasktype->name) && $orderId->tasktype->name == 'Pickup' && isset($taskProof[0]) && $taskProof[0]->otp_requried == 1 && $taskProof[0]->otp == 1) {
                ...
                } else if (!empty($orderId->tasktype->name) && $orderId->tasktype->name == 'Drop' && isset($taskProof[1]) && $taskProof[1]->otp_requried == 1 && $taskProof[1]->otp == 1) {
                    if (!empty($request->otp) && $completionOtp->completion_otp != $request->otp) {
                        $errorMsgOtp = __('Otp Not Match');
                    } else if (empty($request->otp)) {
                        $errorMsgOtp = __('Otp is requried');
                    }
                } else if (!empty($orderId->tasktype->name) && $orderId->tasktype->name == 'Appointment' && isset($taskProof[2]) && $taskProof[2]->otp_requried == 1 && $taskProof[2]->otp == 1) {
                    if (!empty($request->otp) && $completionOtp->completion_otp != $request->otp) {
                        $errorMsgOtp = __('Otp Not Match');
                    } else if (empty($request->otp)) {
                        $errorMsgOtp = __('Otp is requried');
