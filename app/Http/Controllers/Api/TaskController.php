...
class TaskController extends BaseController
{
    ...
    public function updateTaskStatus(Request $request)
    {
        ...
        if ($request->task_status == 4) {
            ...
            $tasks = Task::where('dependent_task_id', $orderId->id)->where('task_status', '!=', 4)
                ->Where('task_status', '!=', 5)
                ->with([
                'location',
                'tasktype',
                'order.customer',
                'order.customer.resources',
                'order.task.location'
            ])
            ...
            $task = Task::where('order_id', $orderId->order_id)->update([
        ...
        $newDetails = Task::where('id', $request->task_id)->with([
            'location',
            'tasktype',
            'pricing',
            'order.customer'
    ...
    public function TaskUpdateReject(Request $request)
    {
        ...
        if ($driver->is_pooling_available == 1) {
            $assigned_orders = Order::where('driver_id', $driver->id)->where('is_cab_pooling', 1)
                ->where('status', 'assigned')
                ->orderBy('id', 'asc')
                ->first();
            $available_seats = (! empty($assigned_orders)) ? $assigned_orders->available_seats : 0;
            if ($available_seats > 0) {
                $previous_seats = Order::where('driver_id', $driver->id)->where('is_cab_pooling', 1)
                    ->where('status', 'assigned')
                    ->sum('no_seats_for_pooling');
        ...
        if ($request->status == 1) {
            ...
            if ($request->type == 'B') {
                ...
                $check = Order::where('id', $request->order_id)->with([
                    'agent',
                    'customer'
                ])->first();
                if (! isset($check)) {
                ...
                Task::where('order_id', $request->order_id)->update([
                    'task_status' => 1
                ]);
                if ($check && $check->call_back_url) {
                    $call_web_hook = $this->updateStatusDataToOrder($check, 2, 1); # task accepted
