...
class RazorpayGatewayController extends BaseController
{
    ...
    {
        $razorpay_creds = PayoutOption::select('credentials', 'test_mode')->where('code', 'razorpay')->where('status', 1)->first();
        //  dd($razorpay_creds);
    ...
    public function razorpay_create_contact(Request $request)
    {
        try{
                ...
                if(isset($agent) && empty($agent->razorpay_contact_json))
                {
                    ...
                    return response()->json(['status'=>'200','data'=>json_decode($agent->razorpay_contact_json)]);
                }
            ...
            if ($validator->fails()) {
                ...
                if(isset($razorpayContact) && !empty($razorpayContact->id) && empty($agent->razorpay_bank_json))
                {
                        ...
                        if(isset($result->id) && !empty($result->id))
                        {
                            ...
                            $agent->save();
                           return response()->json(['status'=>'200','message'=>'Razorpay Account Added Successfuly!']);
                        }
                }else{
                    return response()->json(['status'=>'200','message'=>'Already Done Account']);
                }
