    ...
    function initMap(is_refresh) {
        ...
        for (let i = 0; i < allagent.length; i++) {
            ...
            if (displayagent.agentlog != null && displayagent.agentlog['lat'] != "0.00000000" && displayagent.agentlog[
                    'lat'] != null && displayagent.agentlog['long'] != "0.00000000" && displayagent.agentlog['long'] !=
                null) {
    ...
    function addMarker(location, lables, images, data, type) {
        ...
        if (type == 1) {
            ...
            contentString =
                ...
                '<div class="user_name mb-2 11"><label class="d-block m-0">' + data["name"] +
                '</label><span> <i class="fas fa-phone-alt"></i>' + data["phone_number"] + '</span></div>' +
                '<div><b class="d-block mb-2"><i class="far fa-clock"></i> <span> ' + jQuery.timeago(new Date(data[
                    'agentlog']['created_at'])) +
                ' </span></b> <b><i class="fas fa-mobile-alt"></i> ' + data['agentlog']['device_type'] +
                '</b> <b class="ml-2"> <i class="fas fa-battery-half"></i>  ' + data['agentlog']['battery_level'] +
                '%</b>';
            if (data['get_driver'][0]) {
                contentString += '<a target="_blank" href="fleet/details/' + btoa(data['get_driver'][0]['id']) +
                    '"><b class="d-block mt-2"><i class="fa fa-car"></i><span>' + data['get_driver'][0]['name'] +
                    ' </span></b></a>';
            }
            contentString += '</div>';
            '</div>' +
